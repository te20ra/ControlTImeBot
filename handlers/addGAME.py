...
from keyboards import kb_add_game3, kb_add_game2,kb_add_game1, kb_main
...
async def add_game_start(message: types.Message):
    ...
    await message.reply('Введите название игры', reply_markup=kb_add_game1.button_case_add)
...
async def cancel_handler(message: types.Message, state: FSMContext):
    ...
    if current_state is None:
        return
    await state.finish()
    await message.reply('ОК', reply_markup=kb_add_game3.button_case_add)
...
async def add_game_name(message : types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        if await sqlite_db.sql_search(data['name']) == 0 and len(data['name']) <= 30 and not data['name'].isdigit():
            await FSM_user.next()
...
def register_handlers(dp: Dispatcher):
    dp.register_message_handler(mode_menu, commands=['moder'])
