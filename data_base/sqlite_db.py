...
def sql_start():
    global base, cur
    base = sq.connect('time_control.db')
    cur = base.cursor()
    if base:
...
async def sql_add(state):
    async with state.proxy() as data:
        cur.execute('INSERT INTO data_games VALUES (null, ?, ?, ?, ?, ?, ?, ?)', tuple(data.values()))
        base.commit()


async def sql_read_menu(message):
    return cur.execute('SELECT * FROM data_games WHERE iduser == ? AND deleted == "No"', (message.from_user.id,)).fetchall()
...
async def sql_read_to_start_game(iduser):
    return cur.execute('SELECT * FROM data_games WHERE iduser == ? AND deleted == "No" AND spendtime < maxtime', (iduser,)).fetchall()
...
async def sql_status_active(userid, game):
    cur.execute('UPDATE data_games SET status == "active" WHERE iduser == ? AND name == ?', (userid, game))
    base.commit()
...
async def sql_return_id_sql(userid):
    return cur.execute('SELECT id_sql FROM data_games WHERE iduser == ? AND status = "active" AND deleted == "No" AND last_time == ""',
                       (userid,)).fetchone()[0]
...
async def sql_search(name):
    return cur.execute('SELECT EXISTS(SELECT name FROM data_games WHERE name == ? and deleted == "No")', (name,)).fetchone()[0]